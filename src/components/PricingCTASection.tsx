...
export default function PricingCTASection() {
  ...
  return (
    <section className="bg-[#F3F3EF] mx-4 lg:mx-40">
      <div className="lg:mx-20">
        {/* Orange Border Card */}
        <div className="border-4 lg:border-8 border-orange-500 bg-[#F3F3EF] py-8 lg:py-12 px-4 lg:px-16">
          <div className="text-center">
            {/* Top Text */}
            <p className="text-black text-xl md:text-3xl lg:text-5xl font-extrabold mb-2">{t('topText')}</p>
            
            {/* Main Heading - Orange Italic */}
            <h2 className="text-2xl md:text-4xl lg:text-7xl font-extrabold text-orange-500 italic mb-4">
              {t('heading')}
            </h2>
            
            {/* Subtext */}
            <p className="text-black text-lg md:text-3xl lg:text-5xl font-extrabold whitespace-pre-line mb-6 lg:mb-8">
              {t('subText')}
            </p>

            {/* Price Comparison */}
            <div className="flex flex-col lg:flex-row items-center justify-center gap-2 lg:gap-8 mb-4 lg:mb-6">
            ...
            <div className="flex flex-col lg:flex-row items-center justify-center gap-2 lg:gap-4 mb-4 lg:mb-6">
              <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 49 49" fill="none" className="lg:w-12 lg:h-12">
                <g clipPath="url(#clip0_346_657)">
              ...
              <span className="text-black text-xl md:text-3xl lg:text-5xl font-semibold">{t('todayPriceLabel')}</span>
              <span className="text-orange-500 text-2xl md:text-4xl lg:text-5xl font-bold">{t('todayPriceAmount')}</span>
            </div>

            {/* Countdown */}
            <div className="flex items-center justify-center gap-2 mb-6 lg:mb-8">
              <span className="text-[#2C3A3A] text-sm lg:text-base">{t('countdownLabel')}</span>
              <span className="text-black text-2xl lg:text-4xl font-bold">
                {formatTime(timeLeft.minutes)}:{formatTime(timeLeft.seconds)}
              </span>
              <span className="text-[#2C3A3A] text-sm lg:text-base">{t('countdownUnit')}</span>
            </div>

            {/* CTA Button */}
            <button className="w-full lg:w-auto bg-[#F79155] hover:bg-orange-500 text-white font-semibold px-6 lg:px-10 py-4 transition-all duration-300 inline-flex items-center justify-center gap-3 shadow-lg hover:shadow-xl cursor-pointer mb-4 lg:mb-6 text-sm lg:text-base">
              {t('button')}
              <svg 
            ...
            {/* Disclaimer */}
            <p className="text-gray-500 text-xs lg:text-sm">{t('disclaimer')}</p>
          </div>
        </div>
      </div>
    </section>
